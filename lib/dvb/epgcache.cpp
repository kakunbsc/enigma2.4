...
// 			eFatal("LINE %d descriptor not found in descriptor cache %08x!!!!!!", __LINE__, *(p-1));
			eDebug("LINE %d descriptor not found in descriptor cache %08x!!!!!!", __LINE__, *(p-1));
		tmp-=4;
	}
	ASSERT(pos <= 4108);
...
// 				eFatal("LINE %d descriptor not found in descriptor cache %08x!!!!!!", __LINE__, *(d-1));
				eDebug("LINE %d descriptor not found in descriptor cache %08x!!!!!!", __LINE__, *(d-1));
			ByteSize -= 4;
		}
		delete [] EITdata;
	}
...
//nemesis
void eEPGCache::saveEpg()
{
	save();
}
void eEPGCache::reloadEpg()
{
	load();
}
void eEPGCache::clearEpg()
{
	flushEPG();
}
...
next:
		...
		{
			FILE *f = fopen("/usb/event_map.txt", "w+");
			int i=0;
...
void eEPGCache::load()
{
	...
	char epgFilePath[256];
	nemTool::getInstance()->getRegKey("config.nemepg.path",epgFilePath);
	int i = strlen(epgFilePath);
	if(epgFilePath[i-1]=='\n') epgFilePath[i-1]='\0';

	if (strstr(epgFilePath ,"None"))
		strcpy(epgFilePath,"/usb/epg.dat");
	else
		strcat(epgFilePath,"/epg.dat");

	FILE *f = fopen(epgFilePath, "r");
// 	FILE *f = fopen("/usb/epg.dat", "r");
	if (f)
	{
// 		unlink("/usb/epg.dat");
		unlink(epgFilePath);
...
#endif
		{
			...
			if ( !strncmp( text1, "ENIGMA_EPG_V7", 13) )
			{
				...
				eventData::load(f);
// 				eDebug("[EPGC] %d events read from /usb/epg.dat", cnt);
				eDebug("[EPGC] %d events read from %s", cnt, epgFilePath);
#ifdef ENABLE_PRIVATE_EPG
				char text2[11];
				fread( text2, 11, 1, f);
...
// 		return;
	...
	FILE *f = fopen(epgFilePath, "w");
// 	FILE *f = fopen("/usb/epg.dat", "w");
	int cnt=0;
	if ( f )
	{
		...
		eDebug("[EPGC] %d events written to %s", cnt, epgFilePath) ;
// 		eDebug("[EPGC] %d events written to /usb/epg.dat", cnt);
		eventData::save(f);
#ifdef ENABLE_PRIVATE_EPG
