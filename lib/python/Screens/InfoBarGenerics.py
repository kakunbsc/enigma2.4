...
from DB.dbpExtraInfoBar import dbpEI

from Tools import Notifications
from Tools.Directories import SCOPE_HDD, resolveFilename, fileExists

from enigma import eTimer, eServiceCenter, eDVBServicePMTHandler, iServiceInformation, \
	iPlayableService, eServiceReference, eEPGCache, eDBoxLCD
...
class InfoBarShowHide:
	...
	def __init__(self):
		self.InfoBarExtraDialog = self.session.instantiateDialog(dbpEI)
	...
	def __onShow(self):
		...
		if config.dbp.einfo.value:
			self.activityTimer.start(config.dbp.einfotimeout.value, True)
